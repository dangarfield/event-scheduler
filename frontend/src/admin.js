...
const renderEvents = () => {
  ...
  document.querySelectorAll('.data-event').forEach(eventEle => {
    ...
    eventEle.querySelector('.save-to-excel').addEventListener('click', () => {
      ...
      const event = getEventDataFromTable(eventEle)
      for (const date of event.dates) {
        for (const [slotIndex, slot] of date.slots.entries()) {
                ...
                <div class="accordion-item">
                    ...
                    <div class="accordion-collapse collapse">
                        <div class="accordion-body">
                            <div>
                                <video src='event-scheduler-walkthrough.webm' controls fullscreen style='max-height:300px;' />
                            </div>

                            <p>Note: If you want to change the date of the event, you'll have to create a new one</p>
                            <p>Configuring:</p>
                            <ul>
