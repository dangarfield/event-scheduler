...
export async function handler(req, context) {
  ...
  try {
    ...
    let attendeeID = pathSplit[3]
    try {
      attendeeID = atob(pathSplit[3])
    } catch (error) {
      return {
        statusCode: 400,
        body: JSON.stringify({error:'Bad request'})
    ...
    console.log('ATTENDEE request', attendeeID)
    if (req.httpMethod === 'GET') {
      console.log('get event')
      const eventsCollection = await getEventsCollection()
      let events = await eventsCollection.find({attendees:attendeeID}).toArray()
      ...
      //   }
        ...
        events.sort((a, b) => new Date(a.dates[0].date) - new Date(b.dates[0].date))
